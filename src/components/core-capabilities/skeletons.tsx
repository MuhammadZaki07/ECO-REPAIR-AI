...
import {IconCamera, IconMapPin, IconTrophy, IconUsers } from "@tabler/icons-react";
...
export const SkeletonOne = () => {
  ...
  const MessageRow = ({ variant, icon }: any) => (
    <motion.div
      variants={variant}
      className="flex flex-row rounded-full border border-neutral-100 dark:border-white/[0.2] p-2 items-center space-x-2 bg-neutral-100 dark:bg-black"
    >
      {icon && <div className={`h-6 w-6 rounded-full bg-[${SoftGreen}] shrink-0 flex items-center justify-center`}>{icon}</div>}
      <div className="w-full bg-neutral-200 h-4 rounded-full dark:bg-neutral-900" />
    </motion.div>
  );
  ...
  return (
    <motion.div initial="initial" whileHover="animate" className="flex flex-1 w-full h-full min-h-[6rem] dark:bg-dot-white/[0.1] bg-dot-black/[0.1] flex-col space-y-2">
      <MessageRow variant={variantsA} icon={<IconCamera className="w-5 h-5 dark:text-black text-neutral-700" />} />
    ...
    >
      {arr.map((_, i) => (
        <motion.div
          ...
          className={`flex flex-row rounded-full border border-neutral-100 dark:border-white/[0.1] p-1.5 items-center space-x-2 dark:bg-[#161616] bg-neutral-200 w-full h-4 ${
            i % 3 === 0 ? `bg-[${SoftGreen}]/20` : ""
          }`}
        >
          <div
            className={`h-2 w-2 rounded-full bg-neutral-300 ${
              i % 3 === 0 ? `dark:bg-[${SoftGreen}]` : "dark:bg-white/20"
            }`}
          ></div>
          <div className="w-full dark:bg-white/10 bg-neutral-300 h-1 rounded-full"></div>
        </motion.div>
      ))}
    </motion.div>
  );
...
export const SkeletonThree = () => {
  ...
  return (
    ...
    >
      ...
      <motion.div className="text-6xl font-bold dark:text-white text-neutral-800 tracking-tighter">
        {count.toLocaleString()}
      </motion.div>
      <p className="text-sm dark:text-white/60 text-neutral-700 mt-3">Eco-Coins Balance</p>
    </motion.div>
  );
};
export const SkeletonFour = () => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={first}
        className="h-full w-1/3 rounded-2xl bg-neutral-50 p-4 dark:bg-black dark:border-white/[0.1] border border-neutral-200 flex flex-col items-center justify-center"
      >
        <IconCamera className="h-10 w-10 text-neutral-500" />
        <p className="sm:text-sm text-xs text-center font-semibold text-neutral-500 mt-4">
          Broken Toaster (Before)
        </p>
        <p className="border border-red-500 dark:bg-red-900/20 dark:text-red-400 text-red-800 bg-red-400/20 text-xs rounded-full px-2 py-0.5 mt-4">
          DAMAGED
        </p>
      </motion.div>

      {/* Card 2: FIXED (Center) */}
      <motion.div className="h-full relative z-20 w-1/3 rounded-2xl bg-neutral-50 p-4 dark:bg-black dark:border-white/[0.1] border border-neutral-200 flex flex-col items-center justify-center">
      ...
      >
        ...
        <p className="border border-yellow-500 dark:bg-yellow-900/20 bg-yellow-500/20 dark:text-yellow-400 text-yellow-800 text-xs rounded-full px-2 py-0.5 mt-4">
          COMMUNITY Q
        </p>
      </motion.div>
    </motion.div>
...
export const SkeletonFive = () => {
  return (
    ...
    >
      ...
      <div
        className={`w-full h-32 rounded-xl dark:bg-gradient-to-br from-gray-700 to-gray-900 bg-neutral-100 border border-[${SoftGreen}]/50 flex items-center justify-center`}
      >
        <IconMapPin className="w-8 h-8 text-[#4ade80]" />
      </div>

      <div className="flex flex-row items-center space-x-2 w-full p-1 dark:bg-neutral-900 bg-neutral-100 rounded-md">
