...
import { Leaf, Heart, TrendingUp, CheckCircle2, Info, Sparkles } from "lucide-react";
...
export default function DonationTab({ search }: { search: string }) {
  ...
  const { campaigns, refetch: refetchCampaigns } = useDonationCampaigns();
  ...
  const [selectedDonation, setSelectedDonation] = useState<UIDonation | null>(null);
  ...
  const userHasDonated = (id: string) => records.some((r) => r.campaign_id === id);
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => handleInfoClick(item)}>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold">{item.current} / {item.goal} EC</span>
                    </div>
      ...
      <Dialog open={donationDialogOpen} onOpenChange={setDonationDialogOpen}>
        <DialogContent className="sm:max-w-[450px]">
          <DialogHeader>
            ...
            <DialogDescription>Support {selectedDonation?.title}</DialogDescription>
          </DialogHeader>

          <div className="space-y-4">
            <Alert className="bg-emerald-500/5">
              <Leaf className="h-4 w-4 text-emerald-500" />
              <AlertDescription>Impact: {selectedDonation?.impact}</AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">1 EC = Rp 1.000 • Min 10 EC</p>
            </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg">
              ...
              />
              <Label className="text-sm">I agree this donation will be recorded on the blockchain</Label>
            </div>
            ...
            <div className="flex gap-2">
              <Button
                className="flex-1"
                disabled={
                  submitting || !donationAmount || !agreedToTerms || Number(donationAmount) < 10 || (userBalance ?? 0) < Number(donationAmount)
                }
        ...
        <DialogContent className="sm:max-w-[400px] text-center">
          <div className="space-y-4 py-4">
            ...
            <DialogHeader>
              ...
              <DialogDescription>Thank you for making a difference</DialogDescription>
            </DialogHeader>
            <p className="text-xl font-bold text-emerald-500">{donationAmount} EC</p>
            <Button className="w-full" onClick={() => setSuccessDialogOpen(false)}>Close</Button>
          </div>
        </DialogContent>
      </Dialog>
    </>
